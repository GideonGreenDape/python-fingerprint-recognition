...
def ridge_orient(im, gradientsigma, blocksigma, orientsmoothsigma):
    ...
    gauss = cv2.getGaussianKernel(int(sze), gradientsigma)  # Ensure sze is an int
    f = gauss * gauss.T
    ...
    gauss = cv2.getGaussianKernel(int(sze), blocksigma)  # Ensure sze is an int
    f = gauss * gauss.T
    ...
    if orientsmoothsigma:
        ...
        if sze % 2 == 0:
            sze += 1
        gauss = cv2.getGaussianKernel(int(sze), orientsmoothsigma)  # Ensure sze is an int
        f = gauss * gauss.T
        cos2theta = ndimage.convolve(cos2theta, f)  # Smoothed sine and cosine of doubled angles
